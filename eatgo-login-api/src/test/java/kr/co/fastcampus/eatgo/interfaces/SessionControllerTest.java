...
public class SessionControllerTest {
    ...
    public void createRestaurantOwner() throws Exception {
        ...
        User mockUser=User.builder().name(name).id(id).restaurantId(1004L).level(50L).build();

        given(userService.authenticate(email,password)).willReturn(mockUser);

        given(jwtUtil.createToken(id,name,1004L)).willReturn("header.payload.signature");
